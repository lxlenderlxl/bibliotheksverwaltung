...
(3, 2, NULL, 143, '2009-03-22', 0, 1),
(4, 1, 8, 122, '2009-04-15', 0, 1);

-- --------------------------------------------------------

--
-- Stellvertreter-Struktur des Views `exemplar_retrieve`
--
CREATE TABLE IF NOT EXISTS `exemplar_retrieve` (
`EXEMPLAR_ID` int(11)
,`ZUSTAND_ID` int(11)
,`ZUSTAND_INHALT` char(32)
,`AUSLEIHER_ID` int(11)
,`AUSLEIHER_VORNAME` char(32)
,`AUSLEIHER_NACHNAME` char(32)
,`AUSLEIHER_STRASSE` char(32)
,`AUSLEIHER_HAUSNUMMER` char(8)
,`AUSLEIHER_PLZ` varchar(5)
,`AUSLEIHER_STADT` char(32)
,`AUSLEIHER_AKTIV` tinyint(1)
,`MEDIUM_ID` int(11)
,`MEDIUM_TITEL` char(128)
,`MEDIUM_AUTORVORNAME` char(32)
,`MEDIUM_AUTORNACHNAME` char(32)
,`MEDIUM_VERLAG` char(128)
,`MEDIUM_ERSCHEINUNGSJAHR` int(11)
,`MEDIUM_ISBN` char(32)
,`MEDIUM_AKTIV` tinyint(1)
,`EXEMPLAR_RUECKGABEDATUM` date
,`EXEMPLAR_VERLAENGERUNG` int(11)
,`EXEMPLAR_AKTIV` tinyint(1)
);
-- --------------------------------------------------------
...
(2, 'Medium zurückgegeben'),
(3, 'Medium hinzugefügt'),
(4, 'Medium entfernt'),
...
--
-- Struktur des Views `exemplar_retrieve`
--
DROP TABLE IF EXISTS `exemplar_retrieve`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `bibliotheksverwaltung`.`exemplar_retrieve` AS select `e`.`EXEMPLARID` AS `EXEMPLAR_ID`,`z`.`ZUSTANDSID` AS `ZUSTAND_ID`,`z`.`INHALT` AS `ZUSTAND_INHALT`,`a`.`AUSLEIHERID` AS `AUSLEIHER_ID`,`a`.`VORNAME` AS `AUSLEIHER_VORNAME`,`a`.`NACHNAME` AS `AUSLEIHER_NACHNAME`,`a`.`STRASSE` AS `AUSLEIHER_STRASSE`,`a`.`HAUSNUMMER` AS `AUSLEIHER_HAUSNUMMER`,`a`.`PLZ` AS `AUSLEIHER_PLZ`,`a`.`STADT` AS `AUSLEIHER_STADT`,`a`.`AKTIV` AS `AUSLEIHER_AKTIV`,`m`.`MEDIENID` AS `MEDIUM_ID`,`m`.`TITEL` AS `MEDIUM_TITEL`,`m`.`AUTORVORNAME` AS `MEDIUM_AUTORVORNAME`,`m`.`AUTORNACHNAME` AS `MEDIUM_AUTORNACHNAME`,`m`.`VERLAG` AS `MEDIUM_VERLAG`,`m`.`ERSCHEINUNGSJAHR` AS `MEDIUM_ERSCHEINUNGSJAHR`,`m`.`ISBN` AS `MEDIUM_ISBN`,`m`.`AKTIV` AS `MEDIUM_AKTIV`,`e`.`RUECKGABEDATUM` AS `EXEMPLAR_RUECKGABEDATUM`,`e`.`VERLAENGERUNG` AS `EXEMPLAR_VERLAENGERUNG`,`e`.`AKTIV` AS `EXEMPLAR_AKTIV` from (((`bibliotheksverwaltung`.`exemplar` `e` join `bibliotheksverwaltung`.`zustand` `z` on((`z`.`ZUSTANDSID` = `e`.`ZUSTANDSID`))) join `bibliotheksverwaltung`.`ausleiher` `a` on((`a`.`AUSLEIHERID` = `e`.`AUSLEIHERID`))) join `bibliotheksverwaltung`.`medium` `m` on((`m`.`MEDIENID` = `e`.`MEDIENID`)));
